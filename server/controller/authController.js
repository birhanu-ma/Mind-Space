...
export const login = async (req, res, next) => {
  ...
  // check if a user is exists and password is correct
...
export const isLoggedIn = async (req, res, next) => {
  if (req.cookie.jwt) {
    try {
      ...
      // check if a user changed password after the token was issued
...
export const forgot = async (req, res, next) => {
  ...
  try {
    ...
    if (!user) {
      return next(new AppError("User doesn't exist by this email", 404));
    }
...
export const resetPassword = async (res, req, next) => {
