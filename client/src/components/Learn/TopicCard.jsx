...
const TopicCard = ({id, title, description, variant = 'primary' }) => {
  ...
  return (
    <div className={`bg-white rounded-lg p-8 shadow-md flex flex-col h-full transition-all duration-300 ease-in-out transform hover:translate-y-1 hover:shadow-lg ${variant === 'secondary' ? 'bg-[#fff2f2]' : ''}`}>
      <h3 className="font-poppins text-xl font-semibold mb-6 text-center">{title}</h3>
